...
using SQLTrace.Controls;
using SQLTrace.Logic;
using SQLTrace.VisualStyles;
...
namespace SQLTrace
{
    public partial class MainForm : Form
    {
        ...
        private void ToExceltoolStripButton_Click(object sender, EventArgs e)
        {

            Trace trace = CurrentTrace;
            if (trace != null)
            {
                SaveFileDialog savaDialog = new SaveFileDialog();
                savaDialog.Filter = "Excel|*.xls";
                
                if (savaDialog.ShowDialog() == DialogResult.OK)
                {
                    var table = trace.DataTable;
                    string title = "";


                    FileStream fs = new FileStream(savaDialog.FileName, FileMode.OpenOrCreate);
